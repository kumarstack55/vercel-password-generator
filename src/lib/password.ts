export const GROUPS = {
    upper: { label: '英大文字', chars: 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', hint: 'A–Z' },
    lower: { label: '英小文字', chars: 'abcdefghijklmnopqrstuvwxyz', hint: 'a–z' },
    digits: { label: '数字', chars: '0123456789', hint: '0–9' },
    symbols: { label: '記号', chars: Array.from({ length: 94 }, (_, i) => String.fromCharCode(i + 33)).filter(c => !/[A-Za-z0-9]/.test(c)).join(''), hint: 'ASCII' },
};
...
export const DEFAULTS: Settings = { length: '64', groups: ['upper', 'lower', 'digits'], required: true, excluded: '' };
...
export function validate(s: Settings): Errors {
    ...
    if (!s.groups.length)
        e.groups = '文字種を1つ以上選択してください。';
    else {
        const available = s.groups.map(g => [...GROUPS[g].chars].filter(c => !s.excluded.includes(c)));
        if (available.every(g => !g.length))
            e.excluded = '使用できる文字がありません。文字種または使用禁止文字を変更してください。';
        else if (s.required && available.some(g => !g.length))
            e.excluded = s.groups.filter((_, i) => !available[i].length).map(g => GROUPS[g].label).join('・') + 'がすべて除外されています。使用禁止文字を見直すか、各文字種を含める設定をオフにしてください。';
...
function allocateCounts(length: number, groupCount: number, required: boolean): number[] {
...
export function readSettings(value: unknown): Settings | null {
    ...
    if (typeof s.length !== 'string' || typeof s.excluded !== 'string' || typeof s.required !== 'boolean' || !Array.isArray(s.groups) || !s.groups.every(g => Object.hasOwn(GROUPS, g)) || new Set(s.groups).size !== s.groups.length)
        return null;
    return { length: s.length, excluded: s.excluded, required: s.required, groups: [...s.groups] };
}
